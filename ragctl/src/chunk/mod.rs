...
pub fn chunk_document(
    ...
    while current_start < text.len() {
        ...
        current_start = if chunk_end > config.overlap_chars {
            chunk_end - config.overlap_chars
        } else {
            chunk_end
        };
    }
...
fn find_best_break(
    ...
    let min_pos = start + (config.max_chars * 4 / 5);
    let max_pos = std::cmp::min(start + (config.max_chars * 6 / 5), text.len());

    // Find break points in the window
    ...
    if target < text.len() {
        ...
        let search_text = &text[search_start..std::cmp::min(target + 50, text.len())];
    ...
    // Ultimate fallback: just use the target
    std::cmp::min(target, text.len())
}
